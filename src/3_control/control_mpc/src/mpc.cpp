...
MPC::MPC(ros::NodeHandle &nh) : nh_(nh) {
    pub_closest_point_ = nh_.advertise<visualization_msgs::MarkerArray>(
        "/control/mpc/marker", 1, /* latch */ true);
...
void MPC::runAlgorithm() {
    ...
    control_command_.throttle.data       = static_cast<float>(throttle_cmd)*0.2;
